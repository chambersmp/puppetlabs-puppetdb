...
class puppetdb::database::postgresql (
  Stdlib::Host                                       $listen_addresses,#            = lookup(puppetdb::params::database_host),
  Stdlib::Host                                       $puppetdb_server,         #    = lookup(puppetdb::params::puppetdb_server),
  String[1]                                          $database_name,           #    = lookup(puppetdb::params::database_name),
  String[1]                                          $database_username,       #    = lookup(puppetdb::params::database_username),
  Variant[String[1], Sensitive[String[1]]]           $database_password,       #    = lookup(puppetdb::params::database_password),
  Variant[Stdlib::Port::User, Pattern[/\A[0-9]+\Z/]] $database_port,           #    = lookup(puppetdb::params::database_port),
  Boolean                                            $manage_database,         #    = lookup(puppetdb::params::manage_database),
  Boolean                                            $manage_server,           #    = lookup(puppetdb::params::manage_dbserver),
  Boolean                                            $manage_package_repo,     #    = lookup(puppetdb::params::manage_pg_repo),
  String[2,3]                                        $postgres_version,        #    = lookup(puppetdb::params::postgres_version),
  Boolean                                            $postgresql_ssl_on,       #    = lookup(puppetdb::params::postgresql_ssl_on),
  Stdlib::Absolutepath                               $postgresql_ssl_key_path, #    = lookup(puppetdb::params::postgresql_ssl_key_path),
  Stdlib::Absolutepath                               $postgresql_ssl_cert_path,#    = lookup(puppetdb::params::postgresql_ssl_cert_path),
  Stdlib::Absolutepath                               $postgresql_ssl_ca_cert_path,# = lookup(puppetdb::params::postgresql_ssl_ca_cert_path),
  String[1]                                          $read_database_username,  #    = lookup(puppetdb::params::read_database_username),
  Variant[String[1], Sensitive[String[1]]]           $read_database_password,  #    = lookup(puppetdb::params::read_database_password),
  Optional[Stdlib::Host]                             $read_database_host,      #    = lookup(puppetdb::params::read_database_host),
  Postgresql::Pg_password_encryption                 $password_encryption,     #    = lookup(puppetdb::params::password_encryption),
